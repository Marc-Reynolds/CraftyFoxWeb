...
<#
.SYNOPSIS
    Generate detailed implementation tasks from specification and plan
.DESCRIPTION
    Creates a tasks.md file with prioritized, actionable tasks for development
.PARAMETER Json
    Output results as JSON
.EXAMPLE
    .\generate-tasks.ps1 -Json
#>
...
function Main {
    try {
        Write-Host "🔨 SpecKit Task Generator v1.0.0" -ForegroundColor Cyan
        Write-Host "Generating implementation tasks..." -ForegroundColor Yellow
        ...
        # Read specification and plan content
        $specContent = Get-Content $spec -Raw
        $planContent = Get-Content $plan -Raw
        
        # Generate tasks document
        $tasksPath = Join-Path (Split-Path $spec) "tasks.md"
        Generate-TasksDocument -SpecContent $specContent -PlanContent $planContent -OutputPath $tasksPath
        
        Write-Host "✓ Tasks generated: $tasksPath" -ForegroundColor Green
...
function Generate-TasksDocument {
    param(
        [string]$SpecContent,
        [string]$PlanContent,
        [string]$OutputPath
